...
class ATM(Account):
    ...
    def transfer(self, amount, account):
        self.__balance -= amount
        print (f"\n{amount}SAR has been transformed to account number {account} successfully.")
        print ("New balance: ", self.__balance)
        self.__transaction.append(f"{amount}SAR has been transformed to account number {account} successfully.")
        self.update_balance()
    def display(self):
        super().display()
...
def get_valid_username():
    while True:
        ...
        if check_exit(username) is None:
            return None
        if username in users_dict: # Checks if the username in the dict = is already taken.
...
def take_user():# Creates the account.
    ...
    users_dict[username] = ATM(uid, name, age, username, password, acc_id)
...
def remove_friends(user):
    ...
    name = input("Enter friend's name that you want to delete(Q/q to exit): ")
    if check_exit(name) is None:
        return
...
def admin():
    ...
    if pwd: # if the pwd is correct.
        ...
        if data:
            ...
            return ATM(aID, name, age, username, password, acc_id, balance)
    else:
